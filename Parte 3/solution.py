...
class BAProblem(search.Problem):
    ...
    def actions(self, state):
        ...
        # Create a list with all possibilities independent of the current time of arrival of the vessel
        possible_placements = []
        for vessel_index in range(self.vessel_size):
            if vessels_array[vessel_index][1] == -1:  # Vessel not assigned, skip
                continue
            # Place the vessel in front of the already placed vessels
            for i in range(self.vessels[vessel_index][2]):
                possible_placements.append(((vessels_array[vessel_index][0]+self.vessels[vessel_index][1]), i + vessels_array[vessel_index][1]))

        # Order the list of possible placements based on the the time of arival and the index of the berth
        ...
        for vessel_index in range(self.vessel_size):
            ...
            # Tries to find a valid placement for the vessel based on the possible placements
